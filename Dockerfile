...
maintainer="boyfromcave <boyfromcave@gmail.com>" \
      Description="Dockerized YecShell"
...
RUN cd /tmp && git clone https://github.com/yecdev/yecshell.git && \
    cd yecshell && \
    cargo fetch && \
    cargo build --release
#    cd /tmp && rm -rf yecshell

ENV HOME /Ycash
WORKDIR /Ycash

RUN ln -s /tmp/yecshell/target/release/zecwallet-cli /Ycash/yecshell
